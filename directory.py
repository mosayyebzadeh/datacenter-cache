...
class Directory:
  ...
  def update(self, key, size, location, time):
    ...
    if location not in loc: # miss 
      ...
      self.df.at[key, 'gfreq'] = self.df.loc[key,'gfreq'] + 1 # update global access freq
   

  def insert(self, key, size, location, owner, time):
    self.free_space -= 1
    gfreq = 1
    valid = 1
    self.df.at[key] = [time, size, [location], owner, gfreq, valid, time]
  ...
  def halve_gfreq(self, key):
    self.df.at[key, 'gfreq'] = int(self.df.loc[key, 'gfreq']/2)
