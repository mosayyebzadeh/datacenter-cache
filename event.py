  ...
  if (dc.placement == "consistent"): 
    ...
    if dest in req.path: 
      ...
      if dc.cache_layer[dest].cache.has_key(req.name):
        dc.cache_layer[dest].put(req.name, req.size, env.now, dc)
      else:
        req.path.append("DL")    
        dc.datalake_access()
...
def readReqEvent(req, dc, env):
  yield env.timeout(0)
  #print("%s: Read event is %s, path is %s" %(env.now, req.job.objname, req.path))
  #printCacheStats(dc)
  if dc.cache_layer[req.dest].has_key(req.name): # Local Cache Hit
    dc.cache_layer[req.dest].put(req.name, req.size, env.now, dc)
...
#    yield links[dLink].put(links[dLink].capacity)
  ...
  dc.blk_dir.put(req.name, req, 'writeCache', env.now, dc)
  dc.lock.release()
  completion(req,dc,env)
